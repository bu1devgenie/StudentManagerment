...
public class ScheduleServiceImple implements ScheduleService {
	...
	private final MailProRepository mailProRepository;
	...
	@Override
	public List<TimeTableSlotDto> getScheduleOfWeek(LocalDate startWeek, LocalDate endWeek, List<enumRole> roles, String ms) {
		List<TimeTableSlotDto> schedules = new ArrayList<>();
		if (roles.contains(enumRole.Student)) {
			Student student = studentRepository.findByMssv(ms);
			if (student != null) {
				schedules = sessionRepository.getScheduleOfWeekForStudent(startWeek, endWeek, ms);
			}
		} else if (roles.contains(enumRole.Teacher)) {
