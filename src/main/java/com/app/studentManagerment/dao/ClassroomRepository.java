...
public interface ClassroomRepository extends JpaRepository<ClassRoom, Long> {
	...
	@Query(value = """
			select cl.name from classroom cl
			join semester s on cl.semester_id = s.id
			join course c2 on c2.id = cl.course_id
			where (select CURDATE())<s.start_date and (select CURDATE())>(SELECT DATE_SUB(s.start_date, INTERVAL 10 DAY))
			    and c2.name like :courseName
			""", nativeQuery = true)
	List<String> getClassRoomForRegister(@Param("courseName") String CourseName);
}
